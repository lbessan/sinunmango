...
/** Subject que exige AFIP: /C=AR/O={nombre}/CN={alias}/serialNumber=CUIT {cuit}. */
function subjectAfip(cuit: string, nombre: string, alias: string): forge.pki.CertificateField[] {
...
export function generarKeypairYCsr(cuit: string, nombre: string, alias: string): CsrResult {
  ...
  csr.setSubject(subjectAfip(cuit, nombre, alias))
...
/**
 * Reconstruye el CSR a partir de una clave privada YA existente (mismo par de
 * claves), sin generar una nueva. Sirve para que re-generar la solicitud no
 * pise la key y deje huérfano un certificado ya emitido en AFIP.
 */
export function csrDesdeKeyPem(privateKeyPem: string, cuit: string, nombre: string, alias: string): string {
...
/**
 * ¿El certificado corresponde a esta clave privada? Compara el módulo RSA.
 * Si no matchean, no podríamos firmar el login WSAA con ese cert (la key es otra).
 */
export function certificadoMatcheaKey(certPem: string, privateKeyPem: string): boolean {
  try {
    const cert = forge.pki.certificateFromPem(certPem)
    const key = forge.pki.privateKeyFromPem(privateKeyPem)
    const certPub = cert.publicKey as forge.pki.rsa.PublicKey
    return certPub.n.compareTo(key.n) === 0
  } catch {
    return false
  }
}
