...
// Usa EL MISMO formulario que la página /movimientos/[id]/editar
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          <MovimientoForm
            movimiento={{
              ...
              cuenta_origen:   mov.cuenta_origen ?? null,
              categoria:       mov.categoria ?? null,
              subcategoria:    mov.subcategoria ?? null,
              periodo_tarjeta: mov.periodo_tarjeta ?? null,
            ...
            onSaved={u => {
              onSave({
                ...
                monto:            u.monto,
                moneda:           u.moneda,
                cotizacion:       u.cotizacion,
                conciliado:       u.conciliado,
                monto_estimado:   u.monto_estimado,
                cuenta_origen:    u.cuenta_origen,
                categoria:        u.categoria,
                subcategoria:     u.subcategoria,
                categoria_nombre: u.categoria_nombre,
                categoria_icono:  u.categoria_icono,
                tipo_movimiento:  u.tipo_movimiento,
                periodo_tarjeta:  u.periodo_tarjeta,
              })
              onClose()
            }}
            onCancel={onClose}
            onDeleted={() => { onDelete(); onClose() }}
          />
        </div>
      </div>
    </div>
...
function ImportarPdfModal({ cuentaId, periodo, cierreDay, venceDay, movimientosExistentes, categorias, subcategorias, cuentasFamilia, onImported, onClose }: {
  ...
  const callParsearResumen = async (base64: string, password?: string, savePwdFlag = false): Promise<boolean> => {
    ...
    // Mandamos el monto NATIVO por cuota (no monto_estimado en ARS) + la moneda
    // + la posición de cuota. El server dedup-ea contra estos por
    // monto+moneda+cuota SIN mirar el nombre — así detecta un consumo ya
    // cargado aunque en el resumen figure con otro texto. (Antes mandábamos
    // monto ARS y sin cuotas, y el match dependía de que Claude adivinara por
    // nombre.)
    const movsExist = movimientosExistentes.map(m => ({
      monto:        m.monto,          // USD si moneda USD, ARS si ARS
      moneda:       m.moneda ?? 'ARS',
      fecha:        m.fecha,
      cuotas:       m.cuota_actual,
      ...
      {editando && (
        <EditModal
          ...
          onSave={u => setMovs(prev => {
            // Si lo movieron a otro período o a una cuenta ajena a esta tarjeta,
            // ya no pertenece a esta conciliación: lo sacamos de la lista.
            const salioDelPeriodo = !!u.periodo_tarjeta && u.periodo_tarjeta !== periodo
            const familiaIds = (cuentasFamilia ?? []).map(c => c.id)
            const salioDeLaCuenta = !!u.cuenta_origen
              && familiaIds.length > 0 && !familiaIds.includes(u.cuenta_origen)
            if (salioDelPeriodo || salioDeLaCuenta) return prev.filter(m => m.id !== editando.id)
            return prev.map(m => m.id === editando.id ? { ...m, ...u } : m)
          })}
          onDelete={() => setMovs(prev => prev.filter(m => m.id !== editando.id))}
