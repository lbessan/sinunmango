...
import type { CuentaItem, CategoriaItem } from './analitica-shell'

export function BulkIngresosModal({
  open,
  onClose,
  cuentas,
  categorias,
  ...
  // Mes inicio: el próximo mes del día indicado (o este mes si todavía no llegó)
  const hoy = new Date()
  ...
  // Categorías sólo de ingreso
  const catsIngreso = categorias.filter(c => c.tipo_default === 'Ingreso' || c.tipo_default === null)

  // Cuentas no-tarjeta (no tiene sentido recibir sueldo en tarjeta crédito)
  ...
  const submit = async () => {
    ...
    try {
      ...
      setSaving(false)
      // Refrescar la página para que la analítica vea los nuevos movs
      setTimeout(() => {
        router.refresh()
        onClose()
        setSuccess(null)
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6" onClick={e => e.stopPropagation()}>
        ...
        {success !== null ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-slate-500 mt-1">Se van a reflejar en tu analítica en un momento</p>
          </div>
        ) : (
          <>
